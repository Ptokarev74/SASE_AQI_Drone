...
async def test_concurrent_write_detected():
    ...
    with patch("bridge.ble_client.logger") as mock_log:
...
async def test_consumer_loop_only_write_path():
    ...
    from bridge.config import COMMAND_QUEUE_SIZE
    from bridge.server import enqueue_command_drop_oldest
...
async def test_ble_write_failure_policy_handles_exceptions():
    ...
    from bridge.main import command_consumer_loop
