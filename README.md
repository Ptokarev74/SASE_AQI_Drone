...
This repository contains the `bridge` component, a Python service that securely links the drone's low-latency BLE (Bluetooth Low Energy) telemetry feed to the high-throughput WebSocket API consumed by the frontend Progressive Web App (PWA).
...
python3 -m bridge
```
*Note: Refer to `bridge/ARCHITECTURE.md` (Runbook section) for a complete systemd and Nginx deployment guide.*
...
```
INFO:bridge.server:WebSocket auth mode: disabled, token_rotation_allowed: False
...
*For complete implementation tradeoffs, deployment constraints, and the internal framing protocol, please review the [ARCHITECTURE.md](bridge/ARCHITECTURE.md) and [PROTOCOL.md](PROTOCOL.md) documents.*
