...
async def test_queue_never_exceeds_capacity():
    ...
    for i in range(n_floods):
        api.enqueue_command_drop_oldest(queue, _make_cmd(vx=float(i)))
        size = queue.qsize()
...
async def test_accounting_invariant():
    ...
    recv = api._metrics.commands_received
    enq = api._metrics.commands_enqueued
    drop = api._metrics.commands_dropped_oldest
...
async def test_drop_counter_increases():
    ...
    for i in range(COMMAND_QUEUE_SIZE):
        api.enqueue_command_drop_oldest(queue, _make_cmd(vx=float(i)))
    assert api._metrics.commands_dropped_oldest == 0

    # Now overflow it
    for i in range(20):
        api.enqueue_command_drop_oldest(queue, _make_cmd(vx=float(100 + i)))

    assert api._metrics.commands_dropped_oldest == 20, (
        f"Expected 20 drops, got {api._metrics.commands_dropped_oldest}"
    )
    print(f"  Drops: {api._metrics.commands_dropped_oldest}")
    print("  SUCCESS")
...
async def test_newest_command_preserved():
    ...
    for _ in range(COMMAND_QUEUE_SIZE):
        api.enqueue_command_drop_oldest(queue, _make_cmd(vx=0.0))

    # Send the "freshest" command — should displace the oldest
    fresh_vx = 99.9
    api.enqueue_command_drop_oldest(queue, _make_cmd(vx=fresh_vx))
...
async def test_no_queue_full_during_flood():
    ...
    assert api._metrics.commands_received == n_commands

    print(
        f"  Flooded {n_commands} commands at ~{hz} Hz over {elapsed:.2f}s — "
        f"enqueued={api._metrics.commands_enqueued}, "
        f"dropped={api._metrics.commands_dropped_oldest}"
    )
    print("  SUCCESS")
...
async def test_slow_consumer_queue_capped():
    ...
    assert api._metrics.commands_dropped_oldest > 0, "Expected drops with slow consumer"
    print("  SUCCESS")
