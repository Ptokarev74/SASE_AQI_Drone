...
├── __main__.py          # python -m websocket entrypoint
...
5. **Arguments**: `-m bridge`
6. **Start in**: `C:\Users\lev52\Desktop\SASE_AQI_Drone`
...
WorkingDirectory=/opt/SASE_AQI_Drone
Restart=always
RestartSec=5
...
| **Assignment-only update path** | `ble_client.py` assigns `self.latest_telemetry = msg` and never mutates fields | Broadcasts can never see a half-written object |
...
| Enqueue path | `enqueue_command_drop_oldest()` in `server.py` — **single source of truth** |
...
$env:PYTHONPATH = "C:\Users\lev52\Desktop\SASE_AQI_Drone"
& 'C:\Users\lev52\AppData\Local\Programs\Python\Python313\python.exe' -m bridge
```
